  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        disabled={disabled}
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between gap-2 bg-[var(--input)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-left text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)] transition-colors disabled:opacity-50 cursor-pointer"
      >
      ...
      {open && (
        <div className="absolute z-30 mt-2 w-full max-h-72 overflow-y-auto bg-[var(--surface)] border border-[var(--border)] rounded-xl py-1 custom-scrollbar animate-fadeIn">
          {options.length === 0 ? (
